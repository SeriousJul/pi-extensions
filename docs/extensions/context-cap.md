...
Caps the effective context window for a pi session, so auto-compaction
fires early and prompts stay small:
...
- the cap is at or below `compaction.reserveTokens` (default 16384). A
  cap that small would compact every turn.
